...
//all source base of hexadecimal
...
void hex_to_oct(int num){
//first we have to do hex to binary
string str="";
int k=0;
while(num>0){
	k=0;
...
void convert_any_to_any(int src,int dist,int num){
	int ans=0;
	int k=1;
	ans+=(num%10)*pow(src,0);
	num=num/10;
	while(num>0){
		ans+=(num%10)*pow(src,k);
		k++;
		num=num/10;
	}
	string res="";
...
int32_t main(){
	...
	convert_any_to_any(src,dis,num);
	return 0;   
}